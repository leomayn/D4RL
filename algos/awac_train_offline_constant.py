...
flags.DEFINE_string('env_name', 'halfcheetah-expert-medium-v2', 'Environment name.')
...
def main(_):
    summary_writer = SummaryWriter(
        os.path.join(FLAGS.save_dir, 'tb', FLAGS.env_name, 'constant'))
    ...
    reward = random.normal(key, shape=dataset.rewards.shape) * 100
    ...
    dataset.rewards = jnp.ones_like(dataset.rewards)
    ...
    for i in tqdm.tqdm(range(1, FLAGS.max_steps + 1),
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            np.savetxt(os.path.join(FLAGS.save_dir, f'{FLAGS.env_name}_constant.txt'),
                       eval_returns,
                       fmt=['%d', '%.1f'])
