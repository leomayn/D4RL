...
def brax_evaluate(
    ...
    test_env = envs.get_environment(test_env)
    ...
    _test_rngs = jax.random.split(_test_rng, num_episodes)
    test_env_state  = test_reset_fn(_test_rngs)
    
    # while not jnp.all(test_env_state.done):
    for _ in range(num_steps):
        observation = (test_env_state.obs - obs_mean) / obs_std
        print("brax obs std", observation.std())
        raise ValueError("stop here")
        action = policy_fn(observations=observation)
        test_env_state = test_step_fn(test_env_state, action)
        episode_returns += test_env_state.reward
...
if __name__ == "__main__":
    ...
    for i in tqdm.tqdm(range(1, num_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if i % config.eval_interval == 0:
            ...
            normalized_score = brax_evaluate(
                policy_fn, env, config.test_env_name, config.num_test_episodes, env._max_episode_steps, obs_mean, obs_std, _test_rng
            )
