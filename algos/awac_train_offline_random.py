...
def main(_):
    summary_writer = SummaryWriter(
        os.path.join(FLAGS.save_dir, 'tb', FLAGS.env_name, 'random'))
    ...
    # dataset.rewards = jnp.zeros_like(dataset.rewards)
    ...
    for i in tqdm.tqdm(range(1, FLAGS.max_steps + 1),
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            np.savetxt(os.path.join(FLAGS.save_dir, f'{FLAGS.env_name}_random.txt'),
                       eval_returns,
                       fmt=['%d', '%.1f'])
